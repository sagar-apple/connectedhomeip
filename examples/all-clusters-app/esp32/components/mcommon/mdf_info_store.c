...
esp_err_t mdf_info_erase(const char *key)
{
    ...
    LED_ERROR_CHECK(ret != ESP_OK, ret, "Open non-volatile storage");
    ...
    LED_ERROR_CHECK(ret != ESP_OK && ret != ESP_ERR_NVS_NOT_FOUND,
...
esp_err_t mdf_info_save(const char *key, const void *value, size_t length)
{
    ...
    LED_ERROR_CHECK(ret != ESP_OK, ret, "Set value for given key, key: %s", key);
...
esp_err_t __mdf_info_load(const char *key, void *value, size_t len, uint32_t type)
{
    ...
    if (type == LENGTH_TYPE_NUMBER) {
        ...
        *length = len;
        ...
        ESP_LOGW(TAG, "The type of parameter lenght is incorrect");
        return ESP_ERR_INVALID_ARG;
    }
    ...
    LED_ERROR_CHECK(ret != ESP_OK, ret, "Get value for given key, key: %s", key);
